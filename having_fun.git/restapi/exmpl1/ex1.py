from flask import Flask,jsonify,abort,make_response,request,url_for
from flask_httpauth import HTTPBasicAuth
auth = HTTPBasicAuth()

user = { "nikhil":"nikhil" }

@auth.get_password
def get_pw(username):
 if username in user:
  return user.get(username) 
 return None

@auth.error_handler
def unauthorized():
 return make_response(jsonify({'error':'Unauthorised access'}), 401)

a1=Flask(__name__)
tasks = [
         {
                'id':1, 
		'title': 'Buy Groceries',
		 'description': 'Milk, Cheese, Pizza, Fruit, Tylenol',
		 'done' : False 
	},
	{ 
	 'id':2,
	 'title': 'Learn',
	 'description': 'Where there',
	 'done' : False
	 }
]

@a1.route('/tasks',methods=['GET'])
@auth.login_required
def get_tasks():
 return jsonify({'tasks':tasks})

@a1.route('/tasks/<int:idd>',methods=['GET'])
@auth.login_required
def get_task(idd):
 task = [ task for task in tasks if task['id'] == idd ]
 if len(task) == 0:
  abort(404)
 return jsonify({'task' : task[0]})

@a1.errorhandler(404)
def not_found(error):
 return make_response(jsonify({'error':'Not found'}),404)

@a1.route('/tasks',methods=['POST'])
@auth.login_required
def create_task():
 if not request.json or not 'title' in request.json:
  abort(400)
 task = {
   'id': tasks[-1]['id'] + 1,
   'title': request.json['title'],
   'description':request.json.get('description',''),
   'done': False
        }
 tasks.append(task)
 return jsonify({'task': task}),201

@a1.route('/tasks/<int:id>',methods=['PUT'])
@auth.login_required
def update_task(id):
 task = [task for task in tasks if task['id'] == id]
 if len(task) == 0:
  abort(400)
 if not request.json:
  abort(200)
 if 'title' in request.json and type(request.json['title']) != unicode:
  abort(300)
 if 'description' in request.json and type(request.json['description']) is not unicode:
  abort(400)
 if 'done' in request.json and type(request.json['done']) is not bool:
  abort(400)
 task[0]['title'] = request.json.get('title',task[0]['title'])
 task[0]['description'] = request.json.get('description',task[0]['description'])
 task[0]['done'] = request.json.get('done', task[0]['done'])
 return jsonify({'task':task[0]})

@a1.route('/tasks/<int:id>',methods=['DELETE'])
@auth.login_required
def delete_tasks(id):
 task = [ task for task in tasks if task['id'] == id ]
 if len(task) == 0:
  abort(400)
 tasks.remove(task[0])
 return jsonify({'result': True})

def make_public(task):
 new_task = {} 
 for field in task:
  if field == 'id':
   new_task['uri'] = url_for('get_task', task_id=task['id'], _external=True)
  else:
   new_task[field] = task[field]
 return new_task

#@a1.route('/', methods=['GET'])
#def g_tsks():
# return jsonify({'tasks': [make_public(task) for task in tasks]})


if __name__ == '__main__':
 a1.run(port=5000)
